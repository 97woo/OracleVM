...
impl CoinbaseClient {
        ...
        for attempt in 1..=max_retries {
            ...
            match self.fetch_btc_price_once().await {
                ...
                Err(e) if attempt < max_retries => {
                    ...
                    warn!(
                        "Failed to fetch price from Coinbase (attempt {}): {}. Retrying in {}s...",
                        attempt, e, wait_time
                    );
                ...
                Err(e) => {
                    error!(
                        "Failed to fetch price from Coinbase after {} attempts: {}",
                        max_retries, e
                    );
                    return Err(e);
                }
    ...
    async fn fetch_btc_price_once(&self) -> Result<PriceData> {
        ...
        info!(
            "🎯 Coinbase: Requesting candle for {} UTC",
            target_minute_start.format("%H:%M:%S")
        );

        // 1분 캔들스틱 데이터 요청 (특정 시점)
        let url = format!(
            "{}?start={}&end={}&granularity=60",
        ...
        // 캔들스틱 시간 정보 로깅
        let candle_time = chrono::DateTime::from_timestamp(timestamp as i64, 0).unwrap_or_default();
